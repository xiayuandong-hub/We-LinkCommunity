...
首先这是个只会在用IDEA开发启动时出现的问题，原因是IDEA在启动SpringBoot项目的时候会把项目中的 `resources` 加载到内存里，We-Link中的`static`文件夹在resources里
...
接着把We-Link的表内字段有可能会出现emoji的字段的编码都改成 `utf8mb4_unicode_ci` 这个类型，再发帖就没问题了
