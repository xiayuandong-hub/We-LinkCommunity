...
We-Link开发的插件都在这个plugins目录里，这里放的都是源码，如果想用的话，可以自己通过mvn打包然后使用，用法如下
...
在插件里也可以使用We-Link里已经开发好的service以及mapper
====

插件的实现原理是使用spring的aop切面编程思想实现的，We-Link里提供了service包下几乎所有service方法的切入点
...
[TIP]
====
注意不要将We-Link给打到jar包里了，在引入信赖的时候 `scope` 请配置成 `provided`
====
...
1. 将We-Link主项目打包，命令 `mvn clean package`
2. 将打包好的we-link.jar拷贝到你想放的目录下，比如 `/opt/we-link/`
3. 在`/opt/we-link/`目录下创建一个文件夹 `plugins`
4. 将插件打包好后的jar包拷贝到 `plugins` 目录中
5. 启动项目使用命令 `java -Dloader.path=./plugins -jar we-link.jar`
...
3. 插件包名必须为 `co.yiiu.welink.plugin` （主题插件除外）
4. 插件打包必须要使用jdk8编译
5. 附上插件说明，越详细越好

首先下载We-Link项目，将`pom.xml`里的下面代码注释掉
...
然后运行命令 `mvn clean install` 将We-Link安装到本地maven库，这样在插件里就可以通过下面信赖引入了

[source,xml]
----
<dependency>
    <groupId>co.yiiu</groupId>
    <artifactId>we-link</artifactId>
...
然后在We-Link项目里的pom.xml里引入插件

最后启动We-Link，即可测试插件功能了
