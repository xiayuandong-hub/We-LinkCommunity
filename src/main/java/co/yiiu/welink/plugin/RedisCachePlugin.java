...
public class RedisCachePlugin {
    ...
    @Around("co.yiiu.welink.hook.TopicServiceHook.selectById()")
    ...
    @Around("co.yiiu.welink.hook.TopicServiceHook.updateViewCount()")
    ...
    @Around("co.yiiu.welink.hook.TopicServiceHook.vote()")
    ...
    @After("co.yiiu.welink.hook.TopicServiceHook.update()")
    ...
    @Around("co.yiiu.welink.hook.CommentServiceHook.selectByTopicId()")
    ...
    @After("co.yiiu.welink.hook.CommentServiceHook.insert()")
    ...
    @After("co.yiiu.welink.hook.CommentServiceHook.update()")
    public void commentUpdate(JoinPoint joinPoint) {
        Comment comment = (Comment) joinPoint.getArgs()[0];
        redisService.delString(String.format(RedisKeys.REDIS_COMMENTS_KEY, comment.getTopicId()));
    }

    @After("co.yiiu.welink.hook.CommentServiceHook.delete()")
    public void commentDelete(JoinPoint joinPoint) {
        Comment comment = (Comment) joinPoint.getArgs()[0];
        redisService.delString(String.format(RedisKeys.REDIS_COMMENTS_KEY, comment.getTopicId()));
    }

    @Around("co.yiiu.welink.hook.CommentServiceHook.vote()")
    ...
    @Around("co.yiiu.welink.hook.UserServiceHook.selectByUsername()")
    ...
    @Around("co.yiiu.welink.hook.UserServiceHook.selectByToken()")
    ...
    @Around("co.yiiu.welink.hook.UserServiceHook.selectById()")
    ...
    @After("co.yiiu.welink.hook.UserServiceHook.delRedisUser()")
    ...
    static class RedisKeys {
        public static final String REDIS_TOPIC_KEY = "we_link_topic_%s"; // 后面还要拼上话题的id
        public static final String REDIS_TOPIC_VIEW_IP_ID_KEY = "we_link_topic_view_ip_%s_topic_%s"; // 需要格式化字符串填充上ip地址跟话题id
        public static final String REDIS_COMMENTS_KEY = "we_link_comments_%s"; // 后面还要拼上话题的id

        public static final String REDIS_USER_ID_KEY = "we_link_user_id_%s"; // 后面还要拼上用户的id
        public static final String REDIS_USER_USERNAME_KEY = "we_link_user_username_%s"; // 后面还要拼上用户名
        public static final String REDIS_USER_TOKEN_KEY = "we_link_user_token_%s"; // 后面还要拼上用户的token
    }
}
